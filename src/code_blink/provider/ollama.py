        ...
        async with self._client.stream("POST", f"{self._base}/chat/completions", json=body) as resp:
            if resp.status_code != 200:
                ...
                if resp.status_code == 404:
                    hint = (
                        f"Model '{self.config.model}' not found or not available.\n"
                        f"  • Ollama: ollama pull {self.config.model}\n"
                        f"  • OpenRouter: check https://openrouter.ai/models for available models\n"
                        f"  • LMStudio: ensure the model is loaded"
                    )
