...
def _make_icon(role: str) -> Text:
    ...
    return Text("[CodeBlink] ", style="bold #8b5cf6")
...
class StreamingText(Label):
    ...
    def _build_content(self):
        ...
        status_part = f" [{self._status}]{' ' if self._status else ''}" if self._status else ""
        ts_part = f"  {self._ts}" if not self._status else f"  {self._ts}"
        header = Text.assemble(icon, (status_part, "italic #7c6fa0"), (ts_part, "#5b4b8a"), "\n")
        body = self._buffer
        if body:
            if self._streaming:
